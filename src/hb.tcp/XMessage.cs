...
namespace hb.tcp
{
    ...
    internal class XMessage
    {
        ...
        public byte[] HeaderBytes
        {
            get
            {
                string jsonStr = XSerialization.SerializeJson(this, false);
                byte[] jsonBytes = Encoding.UTF8.GetBytes(jsonStr);
                byte[] end = Encoding.UTF8.GetBytes("\r\n\r\n");
                byte[] final = Utils.AppendBytes(jsonBytes, end);
                return final;
            }
        }
        ...
        private string _Header = "[XMessage] ";
        //                                         1         2         3
        ...
        /// <param name="logger">Logger method.</param>
        internal XMessage(Stream stream, Action<Severity, string> logger)
        {
        ...
        internal async Task<bool> BuildFromStream(CancellationToken token)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    headerBytes = Utils.AppendBytes(headerBytes, headerBuffer); 
                }

                XMessage msg = XSerialization.DeserializeJson<XMessage>(Encoding.UTF8.GetString(headerBytes));
            ...
            catch (Exception e)
            {
                _Logger?.Invoke(Severity.Error, _Header + "exception encountered: " +
                    ...
                    Environment.NewLine +
                    "Exception: " + XSerialization.SerializeJson(e, true) +
                    Environment.NewLine);
                return false;
            }
        }
        ...
        public override string ToString()
        {
            ...
            ret += "  Preshared key     : " + (PresharedKey != null ? Utils.ByteArrayToHex(PresharedKey) : "null") + Environment.NewLine;
