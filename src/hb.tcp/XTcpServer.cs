...
namespace hb.tcp
{
    ...
    /// description:X TCP server, with or without SSL
    public class XTcpServer : IDisposable
    {
        #region Public-Members
     
        /// <summary>
        /// X TCP server settings.
        /// </summary>
        public XTcpServerSettings Settings
        {
            get
            {
                return _Settings;
            ...
            set
            {
                if (value == null) _Settings = new XTcpServerSettings();
        ...
        /// X TCP server events.
        /// </summary>
        public XTcpServerEvents Events
        {
            get
            {
                return _Events;
            ...
            set
            {
                if (value == null) _Events = new XTcpServerEvents();
                else _Events = value;
            }
        }

        /// <summary>
        /// X TCP server callbacks.
        /// </summary>
        public XTcpServerCallbacks Callbacks
        {
            get
            {
                return _Callbacks;
            ...
            set
            {
                if (value == null) _Callbacks = new XTcpServerCallbacks();
        ...
        /// <summary>
        /// Flag to indicate if X TCP is listening for incoming TCP connections.
        /// </summary>
        ...
        private string _Header = "[XTcpServer] ";
        private XTcpServerSettings _Settings = new XTcpServerSettings();
        private XTcpServerEvents _Events = new XTcpServerEvents();
        private XTcpServerCallbacks _Callbacks = new XTcpServerCallbacks();
        ...
        /// <summary>
        /// Initialize the X TCP server without SSL.
        /// Supply a specific IP address on which to listen.  Otherwise, use 'null' for the IP address to listen on any IP address.
        /// If you do not specify an IP address, you may have to run X Tcp with administrative privileges.  
        ...
        /// <summary>
        /// Initialize the X TCP server with SSL.  
        /// Supply a specific IP address on which to listen.  Otherwise, use 'null' for the IP address to listen on any IP address.
        /// If you do not specify an IP address, you may have to run X Tcp with administrative privileges.  
        /// Call Start() afterward to start the server.
        /// </summary>
        ...
        public void Start()
        {
            if (_IsListening) throw new InvalidOperationException("XTcpServer is already running.");
        ...
        public void Stop()
        {
            if (!_IsListening) throw new InvalidOperationException("XTcpServer is not running.");
        ...
        public XResponse SendAndWait(int timeoutMs, string ipPort, string data, Dictionary<object, object> metadata = null)
        {
            byte[] bytes = new byte[0];
            if (!String.IsNullOrEmpty(data)) bytes = Encoding.UTF8.GetBytes(data);
        ...
        /// <param name="start">Start position within the supplied array.</param>
        /// <returns>SyncResponse.</returns>
        public XResponse SendAndWait(int timeoutMs, string ipPort, byte[] data, Dictionary<object, object> metadata = null, int start = 0)
        {
            if (String.IsNullOrEmpty(ipPort)) throw new ArgumentNullException(nameof(ipPort));
        ...
        /// <param name="metadata">Metadata dictionary to attach to the message.</param>
        /// <returns>SyncResponse.</returns>
        public XResponse SendAndWait(int timeoutMs, string ipPort, long contentLength, Stream stream, Dictionary<object, object> metadata = null)
        {
            if (contentLength < 0) throw new ArgumentException("Content length must be zero or greater.");
            if (String.IsNullOrEmpty(ipPort)) throw new ArgumentNullException(nameof(ipPort));
            if (timeoutMs < 1000) throw new ArgumentException("Timeout milliseconds must be 1000 or greater.");
        ...
        public void DisconnectClient(string ipPort, MessageStatus status = MessageStatus.Removed)
        {
            ...
            else
            {
                ...
                XMessage removeMsg = new XMessage();
        ...
        private async Task DataReceiver(ClientMetadata client, CancellationToken token)
        {
            while (true)
            {
                ...
                { 
                    ...
                    XMessage msg = new XMessage(client.DataStream, (_Settings.DebugMessages ? _Settings.Logger : null));
                            ...
                            if (msg.Status == MessageStatus.AuthRequested)
                            {
                                ...
                                if (msg.PresharedKey != null && msg.PresharedKey.Length > 0)
                                {
                                    ...
                                    if (_Settings.PresharedKey.Trim().Equals(clientPsk))
                                    {
                                        ...
                                        Utils.BytesToStream(data, 0, out contentLength, out authStream);
                                        authMsg = new XMessage(null, contentLength, authStream, false, false, null, null, (_Settings.DebugMessages ? _Settings.Logger : null));
        ...
        private bool SendInternal(ClientMetadata client, XMessage msg, long contentLength, Stream stream)
        {
        ...
        private async Task<bool> SendInternalAsync(ClientMetadata client, XMessage msg, long contentLength, Stream stream, CancellationToken token)
        {
            if (client == null) throw new ArgumentNullException(nameof(client));
            if (msg == null) throw new ArgumentNullException(nameof(msg));
        ...
        private XResponse SendAndWaitInternal(ClientMetadata client, XMessage msg, int timeoutMs, long contentLength, Stream stream)
        {
            if (client == null) throw new ArgumentNullException(nameof(client));
            if (msg == null) throw new ArgumentNullException(nameof(msg));
        ...
        private void SendHeaders(ClientMetadata client, XMessage msg)
        { 
        ...
        private async Task SendHeadersAsync(ClientMetadata client, XMessage msg, CancellationToken token)
        { 
            byte[] headerBytes = msg.HeaderBytes;
