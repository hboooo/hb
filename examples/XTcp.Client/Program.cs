...
namespace XTcp.Client
{
    internal class TestClient
    {
        ...
        private static XTcpClient _Client = null;
        ...
        private static void ConnectClient()
        {
            ...
            if (!_Ssl)
            {
                _Client = new XTcpClient(_ServerIp, _ServerPort);
            }
            else
            {
                _Client = new XTcpClient(_ServerIp, _ServerPort, _CertFile, _CertPass);
        ...
        private static void SendAndWait()
        {
            ...
            try
            {
                XResponse resp = _Client.SendAndWait(timeoutMs, userInput, metadata);
        ...
        private static void SendAndWaitEmpty()
        {
            ...
            try
            {
                XResponse resp = _Client.SendAndWait(timeoutMs, "", dict);
                if (resp.Metadata != null && resp.Metadata.Count > 0)
                {
                    Console.WriteLine("Metadata:");
                    foreach (KeyValuePair<object, object> curr in resp.Metadata)
