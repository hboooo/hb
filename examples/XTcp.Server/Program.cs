...
namespace XTcp.Server
{
    internal class TestServer
    {
        ...
        private static XTcpServer _Server = null;
        ...
        private static void Main(string[] args)
        {
            ...
            try
            {
                if (!_Ssl)
                {
                    _Server = new XTcpServer(_ServerIp, _ServerPort);
                }
                else
                {
                    ...
                    _Server = new XTcpServer(_ServerIp, _ServerPort, _CertFile, _CertPass);
        ...
        private static XResponse SyncRequestReceived(XRequest req)
        {
            ...
            return new XResponse(req, retMetadata, "Here is your response!");
        }
        ...
        private static void SendAndWait()
        {
            ...
            try
            {
                XResponse resp = _Server.SendAndWait(timeoutMs, ipPort, userInput);
        ...
        private static void SendAndWaitEmpty()
        {
            ...
            try
            {
                XResponse resp = _Server.SendAndWait(timeoutMs, ipPort, "", dict);
                if (resp.Metadata != null && resp.Metadata.Count > 0)
                {
                    Console.WriteLine("Metadata:");
                    foreach (KeyValuePair<object, object> curr in resp.Metadata)
